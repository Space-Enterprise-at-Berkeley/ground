...
class Navbar extends Component {
  ...
  updateFlightConnected(timestamp, value) {
    this.setState({ flightConnected: value });
  }

  updateDaq1Connected(timestamp, value) {
    this.setState({ daq1Connected: value });
  }

  updateDaq2Connected(timestamp, value) {
    this.setState({ daq2Connected: value });
  }

  updateDaq3Connected(timestamp, value) {
    this.setState({ daq3Connected: value });
  }

  updateActCtrlr1Connected(timestamp, value) {
    this.setState({ actCtrlr1Connected: value });
  }

  updateActCtrlr2Connected(timestamp, value) {
    this.setState({ actCtrlr2Connected: value });
  }

  updateActCtrlr3Connected(timestamp, value) {
    this.setState({ actCtrlr3Connected: value });
  }

  updateFlightKbps(timestamp, value) {
    this.setState({ flightKbps: value });
  }

  updateDaq1Kbps(timestamp, value) {
    this.setState({ daq1Kbps: value });
  }

  updateDaq2Kbps(timestamp, value) {
    this.setState({ daq2Kbps: value });
  }

  updateDaq3Kbps(timestamp, value) {
    this.setState({ daq3Kbps: value });
  }

  updateActCtrlr1Kbps(timestamp, value) {
    this.setState({ actCtrlr1Kbps: value });
  }

  updateActCtrlr2Kbps(timestamp, value) {
    this.setState({ actCtrlr2Kbps: value });
  }

  updateActCtrlr3Kbps(timestamp, value) {
    this.setState({ actCtrlr3Kbps: value });
  }
    ...
    return (
      <AppBar position="static" color="default" elevation={0} className={classes.bar}>
        <Toolbar variant="dense">
          ...
          <Button className={flightConnected ? classes.connectedButton : classes.disconnectedButton}>Flight
            - {flightKbps} kbps</Button>
          <Button className={daq1Connected ? classes.connectedButton : classes.disconnectedButton}>DAQ1
            - {daq1Kbps} kbps</Button>
          <Button className={daq2Connected ? classes.connectedButton : classes.disconnectedButton}>DAQ2
            - {daq2Kbps} kbps</Button>
          <Button className={daq3Connected ? classes.connectedButton : classes.disconnectedButton}>DAQ3
            - {daq3Kbps} kbps</Button>
          <Button className={actCtrlr1Connected ? classes.connectedButton : classes.disconnectedButton}>ActCtrlr1
            - {actCtrlr1Kbps} kbps</Button>
          <Button className={actCtrlr2Connected ? classes.connectedButton : classes.disconnectedButton}>ActCtrlr2
            - {actCtrlr2Kbps} kbps</Button>
          <Button className={actCtrlr3Connected ? classes.connectedButton : classes.disconnectedButton}>ActCtrlr3
            - {actCtrlr3Kbps} kbps</Button>
          <div className={classes.spacer}/>
